...
const CreateRecordForm: React.FC<CreateRecordFormProps> = ({ onRecordCreated, editRecord }) => {
    const { register, handleSubmit, formState: { errors, isDirty }, reset, setError } = useForm<IRecord>({
        ...
        defaultValues: {
            name: '',
            age: null,  // Заміна undefined на null
            email: '',
            question: '',
        },
    });
    ...
    useEffect(() => {
        if (editRecord) {
            // Скидання значень до редагованого запису
            reset(editRecord);
        } else {
            // Скидання до defaultValues при створенні нового запису
            reset({
                name: '',
                age: null,  // Заміна undefined на null
                email: '',
    ...
    const onSubmit = async (data: IRecord) => {
        try {
            ...
            reset();  // скидання форми після успішного додавання/редагування
        } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="p-4 border mb-4 rounded">
            ...
            <button
                type="submit"
                className={`bg-blue-500 text-white px-4 py-2 rounded ${!isDirty ? 'opacity-50 cursor-not-allowed':''}`}
